  ...
  def self.spot_scrape
    ...
    20.times do |i|
      ...
      3.times do |i|
        ...
        column_names = %w(name time location feature image url latitude longitude)
        rows = []
        rows << column_names
        ...
        doc.css('.main-left-layout').each do | node |
          ...
          url = node.at_css('#information p a')["href"].value
          latitude = latitude
          longitude = longitude

          column_values = %w(name time location feature image url latitude longitude)
          rows << column_values
        end
