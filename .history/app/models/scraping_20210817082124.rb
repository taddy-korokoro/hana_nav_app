  ...
  def self.flower_item_scrape
    CSV.open("list.csv", "w") do |csv|
      ...
      doc.css('.flower').each do | node |
        sleep 1
        p node.at_css('.name').inner_text.delete("の名所" "お花見" "まつり" "・")
        p node.at_css('.time').inner_text
        p node.at_css('.description').inner_text
        p node.at_css('img').attribute('src')
        csv << [name,time,feature,image]
      end
    end
  end
