...
require 'mechanize'
  ...
  def self.scrape
    ...
    elements_feature = page.search('div.description') #div.idxcol aは取得したい要素  elementsは任意の変数
    elements_feature.each do |element|
      flower_items = FlowerItem.new                      #Fishは任意のクラス、fishは任意のインスタンス
      flower_items.feature = element.inner_text
      flower_items.save
    end
  end
end
