class Scraping < ApplicationRecord
  ...
  def self.spot_scrape
    ...
    url = "#{base_url}features/hananomeisho?flower=229"
    html = URI.open(url).read
    doc = Nokogiri::HTML.parse(html)

    doc.css('.flower-spot').each do | link |
      sleep 1
      next_link = doc.at_css("a[class=#{link}]")
      ...
      doc.css('.main-left-layout').each do | node |
        ...
        Spot.create!(
          ...
          location: node.at_css('.t-row').inner_text,
          feature: node.at_css('.text > p').inner_text,
