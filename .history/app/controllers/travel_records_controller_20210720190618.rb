class TravelRecordsController < ApplicationController
  ...
  def new
    ...
    @travel_record = @spot.travel_records.new
  end

  def create
    @spot = Spot.find(params[:spot_id])
    @travel_record = @spot.travel_records.new(travel_record_params)
    ...
    if @travel_record.save
      redirect_to spot_travel_record_path(@travel_record.spot_id, @travel_record.id), notice: "投稿に成功しました。"
    else
      render :new
    end
  end
  ...
  private
    def travel_record_params
      params.require(:travel_record).permit(:title, :content, :place, :review,
        ...
        :image_1_cache, :image_2_cache, :image_3_cache, :latitude, :longitude).merge(spot_id: params[:spot_id], user_id: current_user.id )
    end
