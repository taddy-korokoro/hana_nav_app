class TravelRecordsController < ApplicationController
  ...
  def index
    @travel_records = TravelRecord.all.page(params[:page]).per(1).order("created_at DESC")
  end
  ...
  def show
    ...
    @comment = Comment.new
    # 新着順で表示
    @comments = @travel_record.comments.page(params[:page]).per(3).order(created_at: :desc)
    # ページネーションの表示
    # @comments_pagenate = @travel_record.comments.page(params[:page]).per(3).order("created_at DESC")
  end
