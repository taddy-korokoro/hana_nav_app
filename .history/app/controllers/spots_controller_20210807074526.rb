class SpotsController < ApplicationController
  ...
  def show
    @travel_record_first = @spot.travel_records.first
  end
  ...
  def photos
    @travel_records = @spot.travel_records.page(params[:page]).per(5).order(created_at: :desc)
  end
  ...
  private
    ...
    def set_travel_record
      # @travel_record = @spot.travel_records.find(params[:id])
      @travel_record = TravelRecord.find(params[:spot_id])
    end
