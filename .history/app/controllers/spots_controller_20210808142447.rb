class SpotsController < ApplicationController
  ...
  def show
    @travel_records = @spot.travel_records.find(params[:id])
    # 1番目の投稿を取得
    @travel_record_first = @travel_records.first
  end
  ...
  def photos
    # 画像の投稿がない投稿のみを取得
    @travel_records = @spot.travel_records.where.not(image_1: nil).page(params[:page]).per(6).order(created_at: :desc)
  end

  def placards
    @travel_records = @spot.travel_records.page(params[:page]).per(3).order(created_at: :desc)
  ...
  private
    ...
    def set_travel_record
      @travel_record = @spot.travel_records.find(params[:travel_record_id]) if params[:travel_record_id].present?
      # @travel_record = TravelRecord.find(params[:id])
    end

    def review_average
      @review_average = TravelRecord.average(:review).to_f.round(1)
    end
end